...
class DriveTools:
    ...
    Locator schema (for suggest_edit)
    ---------------------------------
      A small dict describing *where* to insert:

      • {"mode": "after_text", "text": "...", "occurrence": 1}
          Find the Nth occurrence (default 1) of literal `text`
          and insert *after* it.

      • {"mode": "after_heading", "heading": "Introduction"}
          Find the first paragraph styled as a heading (Heading 1..6, Title,
          Subtitle) whose text contains `heading` (case-insensitive substring),
          and insert *after* it.
    ...
    def suggest_edit(self, document_id: str, locator: Dict[str, Any], suggestion_text: str) -> Dict[str, Any]:
        ...
        locator : Dict[str, Any]
            Where to insert (see class docstring “Locator schema”).
        suggestion_text : str
...
if __name__ == "__main__":
    ...
    Quick manual test:
      - Creates a Doc,
      - Inserts some starter text,
      - Suggests an edit after a known phrase (highlighted),
      - Reads the Doc back as plain text,
      - Searches Drive for the new file by name.

    Usage:
      python drive_tools.py
    """
    tools = DriveTools()

    # 1) Create a new doc
    name = "DriveTools Demo Doc"
    doc_id = tools.create_google_doc(name)
    print(f"[create_google_doc] Created: {doc_id}")

    # 2) Seed content so our locator has something to match
    seed_text = "Intro line.\nHere is a target sentence.\n\n"
    tools._insert_text(document_id=doc_id, index=1, text=seed_text)
    print("[seed] Inserted starter text")

    # 3) Suggest an edit after the target sentence (highlighted)
    locator = {"mode": "after_text", "text": "Here is a target sentence.", "occurrence": 1}
    suggestion = "[AGENT SUGGESTION] Consider adding a brief summary of objectives here.\n"
    result = tools.suggest_edit(document_id=doc_id, locator=locator, suggestion_text=suggestion)
    print(f"[suggest_edit] Inserted at index {result['insert_index']}")

    # 4) Read the doc back as plain text
    text = tools.get_file_as_text(doc_id)
    print("[get_file_as_text] First 200 chars:")
    print(text[:200].replace("\n", "\\n"))

    # 5) Search for the doc by name
    files = tools.search_files(f"name = '{name}' and mimeType='application/vnd.google-apps.document'", page_size=5)
    print(f"[search_files] Found {len(files)} match(es):")
    for f in files:
        print("  -", f["id"], f["name"], f["mimeType"], f["modifiedTime"])